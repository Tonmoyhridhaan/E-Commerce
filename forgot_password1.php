                        ...
                        <form method="post" class="cmxform" action="" id="ReserPassword">
                            <div class="mb-3 mt-3">
                                <label for="contact">Your Contact</label>
                                    <input type="text" name="contact" class="form-control" id="contact" minlength="2" >
                            </div> 
                            ...
                            <?php
                                if(isset($_POST['submit'])){
                                    ...
                                    //echo $email;
                                    $query = "select * from customers where email = '$email'   and mobile = '$contact'";
                                    $result = mysqli_query($con,$query);
                                    $row = mysqli_fetch_array($result);  
                                    if($row){
                                        $var ='forgot_password2.php?contact='.$contact.'&email='.$email;
                                        echo "<script>window.location.href='".$var."'</script>";
                                         
                                    } 
                                    else{
                                        echo "<span style = 'color:red;'>Invalid mail";
                                    } 
